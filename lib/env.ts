...
export const publicEnv = {
  ...
  celoContractAddress: read(process.env.NEXT_PUBLIC_COSIGN_CELO_CONTRACT_ADDRESS) as Address | "",
  celoDeploymentBlock: read(process.env.NEXT_PUBLIC_COSIGN_CELO_DEPLOYMENT_BLOCK),
  ...
  stacksContractAddress: read(process.env.NEXT_PUBLIC_COSIGN_STACKS_CONTRACT_ADDRESS),
  stacksContractName: read(process.env.NEXT_PUBLIC_COSIGN_STACKS_CONTRACT_NAME, "cosign-registry"),
...
export const contractConfigured = (network: Network) => network === "celo"
  ? Boolean(publicEnv.celoContractAddress)
  : Boolean(publicEnv.stacksContractAddress && publicEnv.stacksContractName);

export const txExplorerUrl = (network: Network, hash: string) => network === "celo"
  ? `${getCeloExplorer()}/tx/${hash}`
  : `https://explorer.stacks.co/txid/${hash.startsWith("0x") ? hash : `0x${hash}`}?chain=${publicEnv.stacksNetwork}`;
