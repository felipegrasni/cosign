      ...
      <header className="app-topbar">
        ...
        <div className="app-actions">
          <details className="network-menu">
            <summary><span className={`network-dot ${network}`} />{network === "celo" ? "Celo" : "Stacks"}<ChevronDown size={15} /></summary>
            <div><Link href="/app/celo">Celo</Link><Link href="/app/stacks">Stacks</Link></div>
          </details>
          {onCreate ? <button type="button" className="button compact" onClick={onCreate}><Plus size={17} /> New</button> : null}
          {account ? (
            <details className="wallet-menu">
              <summary><AddressGlyph address={account} size={34} /><span>{shortAddress(account, 4)}</span></summary>
              <div><Link href={`/app/${network}/profile/${account}`}>Public profile</Link><button type="button" onClick={onDisconnect}><LogOut size={15} /> Disconnect</button></div>
            </details>
          ) : isMiniPay ? <span className="connecting-note">Connecting MiniPay…</span> : (
            <button type="button" className="button secondary compact" onClick={onConnect} disabled={connecting}><Wallet size={17} /> {connecting ? "Connecting" : "Connect"}</button>
          )}
        </div>
      </header>
