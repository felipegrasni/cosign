...
export function Dashboard({ network }: { network: Network }) {
  ...
  const wizardDialogId = `${idPrefix}-create-wizard`;
  const shareDialogId = `${idPrefix}-share-sheet`;
  ...
  return (
    <AppShell network={network} account={client.account} connecting={client.connecting} isMiniPay={client.isMiniPay} onConnect={() => void client.connect()} onDisconnect={() => void client.disconnect()} onCreate={client.connected && client.repository.configured ? () => setWizard(true) : undefined} createDialogId={wizardDialogId} createDialogOpen={wizard}>
      <section className="dashboard-hero">
        ...
        {client.connected && client.repository.configured ? <button type="button" className="button hero-create" onClick={() => setWizard(true)} aria-haspopup="dialog" aria-controls={wizardDialogId} aria-expanded={wizard}><Plus aria-hidden="true" /> Create a CoSign</button> : null}
      </section>

      {!client.repository.configured ? <section className="state-panel"><Signal aria-hidden="true" /><h2>Contract connection pending</h2><p>This {network === "celo" ? "Celo" : "Stacks"} route is ready for its deployed contract address. No demo records are shown here.</p></section> : !client.connected ? <section className="state-panel"><Wallet aria-hidden="true" /><h2>Bring your wallet</h2><p>Connect to create cards and see the moments tied to your address. Public cards and wallet profiles still open by link without connecting.</p>{client.isMiniPay ? <span role="status" aria-live="polite">Connecting MiniPay…</span> : <button type="button" className="button" onClick={() => void client.connect()} disabled={client.connecting} aria-busy={client.connecting} aria-label={client.connecting ? `Connecting ${network === "celo" ? "Celo" : "Stacks"} wallet` : `Connect ${network === "celo" ? "Celo" : "Stacks"} wallet`}>{client.connecting ? `Connecting ${network === "celo" ? "Celo" : "Stacks"}…` : `Connect ${network === "celo" ? "Celo" : "Stacks"}`} <ArrowRight aria-hidden="true" /></button>}</section> : <>
        <div className="dashboard-tabs">
          <nav className="dashboard-tablist" aria-label="Card views" role="tablist">
            <button type="button" id={createdTabId} role="tab" tabIndex={tab === "created" ? 0 : -1} className={tab === "created" ? "active" : ""} aria-selected={tab === "created"} aria-controls={createdPanelId} aria-label={`Show created cards (${created.total})`} onClick={() => setTab("created")}>Created <span>{created.total}</span></button>
            <button type="button" id={signedTabId} role="tab" tabIndex={tab === "signed" ? 0 : -1} className={tab === "signed" ? "active" : ""} aria-selected={tab === "signed"} aria-controls={signedPanelId} aria-label={`Show co-signed cards (${signed.total})`} onClick={() => setTab("signed")}>Co-signed <span>{signed.total}</span></button>
            <button type="button" id={pendingTabId} role="tab" tabIndex={tab === "pending" ? 0 : -1} className={tab === "pending" ? "active" : ""} aria-selected={tab === "pending"} aria-controls={pendingPanelId} aria-label={`Show invitations awaiting a co-signature (${pending.length})`} onClick={() => setTab("pending")}>Awaiting co-sign <span>{pending.length}</span></button>
          </nav>
