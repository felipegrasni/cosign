    ...
    <AppShell network={network} account={client.account} connecting={client.connecting} isMiniPay={client.isMiniPay} onConnect={() => void client.connect()} onDisconnect={() => void client.disconnect()}>
      ...
      {!client.repository.configured ? <section className="state-panel"><AlertTriangle aria-hidden="true" /><h1>Contract connection pending</h1><p>This route needs its deployed {networkLabel} contract address.</p></section> : loading ? <section className="state-panel" role="status" aria-live="polite"><span className="loader" /><p>Reading the signal…</p></section> : error ? <section className="state-panel"><AlertTriangle aria-hidden="true" /><h1>Could not load this card</h1><p>{error}</p><button type="button" className="button secondary" onClick={() => void load()} aria-label="Reload this CoSign card"><RefreshCw aria-hidden="true" /> Reload card</button></section> : !card ? <section className="state-panel"><AlertTriangle aria-hidden="true" /><h1>CoSign not found</h1><p>Check the invitation link and selected network.</p></section> : <section className={`receipt status-${status}`}>
        ...
        <div className="receipt-actions">
          ...
          {isCreator && status === "pending" ? <><button type="button" className="button" onClick={() => setShare(true)} aria-label="Share this invitation link" aria-haspopup="dialog"><Share2 aria-hidden="true" /> Share invitation</button><button type="button" className="button danger" onClick={() => void act("cancel")} disabled={Boolean(action)} aria-busy={cancelBusy} aria-label={cancelBusy ? "Cancelling this invitation through your wallet" : "Cancel this invitation through your wallet"}>{cancelBusy ? actionLabel : "Cancel invitation"}</button></> : null}
          {status === "completed" ? <button type="button" className="button" onClick={() => setShare(true)} aria-label="Share this mutual receipt" aria-haspopup="dialog"><Share2 aria-hidden="true" /> Share receipt</button> : null}
        </div>
      </section>}
