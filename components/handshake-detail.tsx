...
import { canonicalHandshakeUrl, formatMoment, formatMomentDateTime, getStatus, kindLabel, shortAddress } from "@/lib/cosign";
...
export function HandshakeDetail({ network, id }: { network: Network; id: bigint }) {
  ...
  const createdDateTime = card ? formatMomentDateTime(card.createdAt) : undefined;
  const expiresDateTime = card ? formatMomentDateTime(card.expiresAt) : undefined;
  const completedDateTime = card?.completedAt ? formatMomentDateTime(card.completedAt) : undefined;
  const shareDialogId = `handshake-share-${network}-${id.toString()}`;
  ...
  return (
    <AppShell network={network} account={client.account} connecting={client.connecting} isMiniPay={client.isMiniPay} onConnect={() => void client.connect()} onDisconnect={() => void client.disconnect()}>
      ...
      {!client.repository.configured ? <section className="state-panel"><AlertTriangle aria-hidden="true" /><h1>Contract connection pending</h1><p>This route needs its deployed {networkLabel} contract address.</p></section> : loading ? <section className="state-panel" role="status" aria-live="polite"><span className="loader" /><p>Reading the signal…</p></section> : error ? <section className="state-panel"><AlertTriangle aria-hidden="true" /><h1>Could not load this card</h1><p>{error}</p><button type="button" className="button secondary" onClick={() => void load()} aria-label="Reload this CoSign card"><RefreshCw aria-hidden="true" /> Reload card</button></section> : !card ? <section className="state-panel"><AlertTriangle aria-hidden="true" /><h1>CoSign not found</h1><p>Check the invitation link and selected network.</p></section> : <section className={`receipt status-${status}`}>
        <div className="receipt-top"><span className="category"><CategoryIcon kind={card.kind} /> {kindLabel(card.kind)}</span><span className="status-stamp" role="status" aria-live="polite" aria-atomic="true" aria-label={`CoSign card status: ${statusLabel}`}>{statusLabel}</span></div>
        ...
        <dl className="receipt-meta"><div><dt>Created</dt><dd><time dateTime={createdDateTime}>{formatMoment(card.createdAt)}</time></dd></div><div><dt>Expires</dt><dd><time dateTime={expiresDateTime}>{formatMoment(card.expiresAt)}</time></dd></div><div><dt>Completed</dt><dd>{completedDateTime ? <time dateTime={completedDateTime}>{completedMeta}</time> : completedMeta}</dd></div></dl>
        ...
        <div className="receipt-actions">
          ...
          {isCreator && status === "pending" ? <><button type="button" className="button" onClick={() => setShare(true)} aria-label="Share this invitation link" aria-haspopup="dialog" aria-controls={shareDialogId} aria-expanded={share}><Share2 aria-hidden="true" /> Share invitation</button><button type="button" className="button danger" onClick={() => void act("cancel")} disabled={Boolean(action)} aria-busy={cancelBusy} aria-label={cancelBusy ? "Cancelling this invitation through your wallet" : "Cancel this invitation through your wallet"}>{cancelBusy ? actionLabel : "Cancel invitation"}</button></> : null}
          {status === "completed" ? <button type="button" className="button" onClick={() => setShare(true)} aria-label="Share this mutual receipt" aria-haspopup="dialog" aria-controls={shareDialogId} aria-expanded={share}><Share2 aria-hidden="true" /> Share receipt</button> : null}
        </div>
      </section>}
      {share ? <ShareSheet url={canonicalHandshakeUrl(publicEnv.appUrl, network, id)} explorerUrl={lastTx ? txExplorerUrl(network, lastTx) : contractExplorerUrl(network)} variant={status === "completed" ? "receipt" : "invitation"} onClose={() => setShare(false)} dialogId={shareDialogId} /> : null}
    </AppShell>
  );
}
