...
export function ProfileView({ network, address }: { network: Network; address: string }) {
  ...
  return (
    <AppShell network={network} account={client.account} connecting={client.connecting} isMiniPay={client.isMiniPay} onConnect={() => void client.connect()} onDisconnect={() => void client.disconnect()}>
      ...
      <section className="profile-head"><AddressGlyph address={address} size={76} /><div><span className="eyebrow">Public wallet history · {networkLabel}</span><h1 title={address}><span className="sr-only">Wallet address {address}</span><span aria-hidden="true">{shortAddress(address, 8)}</span></h1><p>{created.total} public cards created · {signed.total} public cards co-signed on {networkLabel}</p></div></section>
      {!client.repository.configured ? <section className="state-panel"><Inbox aria-hidden="true" /><h2>Contract connection pending</h2><p>This public profile will load once the deployed {networkLabel} contract address is configured for this route.</p></section> : <div className="profile-columns"><section><header><h2>Created</h2><span>{created.total}</span></header>{created.loading ? <div className="state-panel compact" role="status" aria-live="polite"><span className="loader" /><p>Loading created cards…</p></div> : created.items.length ? <><div className="card-stack">{created.items.map((item) => <HandshakeCard key={item.id} handshake={item} />)}</div><Pagination previous={created.hasPrevious} next={created.hasNext} onPrevious={() => created.setPage(created.page - 1)} onNext={() => created.setPage(created.page + 1)} /></> : <div className="empty-mini">No public cards created on {networkLabel} yet.</div>}</section><section><header><h2>Co-signed</h2><span>{signed.total}</span></header>{signed.loading ? <div className="state-panel compact" role="status" aria-live="polite"><span className="loader" /><p>Loading co-signed cards…</p></div> : signed.items.length ? <><div className="card-stack">{signed.items.map((item) => <HandshakeCard key={item.id} handshake={item} />)}</div><Pagination previous={signed.hasPrevious} next={signed.hasNext} onPrevious={() => signed.setPage(signed.page - 1)} onNext={() => signed.setPage(signed.page + 1)} /></> : <div className="empty-mini">No public cards co-signed on {networkLabel} yet.</div>}</section></div>}
    </AppShell>
  );
}
