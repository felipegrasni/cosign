...
export function ProfileView({ network, address }: { network: Network; address: string }) {
  ...
  return (
    <AppShell network={network} account={client.account} connecting={client.connecting} isMiniPay={client.isMiniPay} onConnect={() => void client.connect()} onDisconnect={() => void client.disconnect()}>
      ...
      <section className="profile-head"><AddressGlyph address={address} size={76} /><div><span className="eyebrow">Public wallet history · {networkLabel}</span><h1 title={address}><span className="sr-only">Wallet address {address}</span><span aria-hidden="true">{shortAddress(address, 8)}</span></h1><p>{created.total} cards created · {signed.total} cards co-signed</p></div></section>
