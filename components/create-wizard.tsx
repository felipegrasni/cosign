...
export function CreateWizard({ network, account, repository, onClose, onCreated, dialogId }: {
  ...
  const signerHintId = "signer-address-hint";
  const titleId = useId();
    ...
    >
      <section id={dialogId} ref={wizardRef} className="wizard" role="dialog" aria-modal="true" aria-labelledby={titleId} aria-describedby={descriptionId}>
        ...
        {step === 2 ? <div className="form-stack">
          <label>Context <span>{cleanAscii(context, CONTEXT_LIMIT).length}/{CONTEXT_LIMIT}</span><input required value={context} maxLength={CONTEXT_LIMIT} onChange={(event) => setContext(event.target.value)} placeholder="ETH Lisbon · Open source lounge" aria-required="true" aria-invalid={Boolean(validation.errors.context && context)} aria-describedby={validation.errors.context && context ? contextErrorId : undefined} />{validation.errors.context && context ? <small id={contextErrorId} className="field-error">{validation.errors.context}</small> : null}</label>
          <label>Note <span>{cleanAscii(note, NOTE_LIMIT).length}/{NOTE_LIMIT}</span><textarea required value={note} maxLength={NOTE_LIMIT} onChange={(event) => setNote(event.target.value)} placeholder="We paired on the release flow and got it over the line." aria-required="true" aria-invalid={Boolean(validation.errors.note && note)} aria-describedby={validation.errors.note && note ? noteErrorId : undefined} />{validation.errors.note && note ? <small id={noteErrorId} className="field-error">{validation.errors.note}</small> : null}</label>
          <p className="field-hint">Everything you enter here becomes public and permanent once this CoSign is published on-chain.</p>
          <fieldset><legend>Who can co-sign?</legend><div className="segmented" role="radiogroup" aria-label="Who can co-sign"><button type="button" role="radio" className={mode === "open" ? "active" : ""} aria-checked={mode === "open"} onClick={() => setMode("open")}><Radio size={17} aria-hidden="true" /> Open link</button><button type="button" role="radio" className={mode === "addressed" ? "active" : ""} aria-checked={mode === "addressed"} onClick={() => setMode("addressed")}><LockKeyhole size={17} aria-hidden="true" /> One wallet</button></div><p className="field-hint">{mode === "open" ? "Anyone with the invitation link on this network can co-sign it." : "Only the wallet address you enter below can co-sign this invitation."}</p></fieldset>
          {mode === "addressed" ? <label>Signer address<input required value={intendedSigner} onChange={(event) => setIntendedSigner(event.target.value.trim())} placeholder={network === "celo" ? "0x..." : "SP..."} autoCapitalize="none" autoCorrect="off" spellCheck={false} aria-required="true" aria-invalid={Boolean(intendedSigner && !addressValid)} aria-describedby={intendedSigner && !addressValid ? `${signerHintId} ${signerErrorId}` : signerHintId} /><small id={signerHintId} className="field-hint">Use a {network === "celo" ? "Celo" : "Stacks"} address for the wallet that should co-sign this card.</small>{intendedSigner && !addressValid ? <small id={signerErrorId} className="field-error">Enter a valid {network === "celo" ? "Celo" : "Stacks"} address.</small> : null}</label> : null}
        ...
        {step === 3 ? <div className="review-card">
          ...
          <div className="public-warning"><LockKeyhole size={20} aria-hidden="true" /><p><strong>Public and permanent.</strong> This text and both wallet addresses stay readable on-chain. They cannot be edited or deleted.</p></div>
