    ...
    <div className="modal-backdrop" role="presentation">
      <section className="wizard" role="dialog" aria-modal="true" aria-labelledby="wizard-title">
        ...
        {step === 2 ? <div className="form-stack">
          ...
          <fieldset><legend>Who can co-sign?</legend><div className="segmented"><button type="button" className={mode === "open" ? "active" : ""} aria-pressed={mode === "open"} onClick={() => setMode("open")}><Radio size={17} /> Open link</button><button type="button" className={mode === "addressed" ? "active" : ""} aria-pressed={mode === "addressed"} onClick={() => setMode("addressed")}><LockKeyhole size={17} /> One wallet</button></div></fieldset>
          ...
          <fieldset><legend>Expires</legend><div className="segmented">{expiryOptions.map((item) => <button type="button" key={item.seconds} className={expiry === item.seconds ? "active" : ""} aria-pressed={expiry === item.seconds} onClick={() => setExpiry(item.seconds)}>{item.label}</button>)}</div></fieldset>
        </div> : null}
        ...
        <footer>{step > 1 ? <button type="button" className="button secondary" onClick={() => setStep(step - 1)} disabled={submitting}><ArrowLeft size={18} /> Back</button> : <span />}{step < 3 ? <button type="button" className="button" onClick={() => setStep(step + 1)} disabled={step === 2 && (Boolean(validation.errors.context || validation.errors.note) || !addressValid)}>Continue <ArrowRight size={18} /></button> : <button type="button" className="button" onClick={submit} disabled={submitting}>{submitting ? submitLabel : <>Create CoSign <Check size={18} /></>}</button>}</footer>
      </section>
    </div>
  );
}
