    ...
    <div className="modal-backdrop" role="presentation">
      <section className="wizard" role="dialog" aria-modal="true" aria-labelledby="wizard-title" aria-describedby={descriptionId}>
        ...
        {step === 2 ? <div className="form-stack">
          ...
          {mode === "addressed" ? <label>Signer address<input required value={intendedSigner} onChange={(event) => setIntendedSigner(event.target.value.trim())} placeholder={network === "celo" ? "0x..." : "SP..."} aria-required="true" aria-invalid={Boolean(intendedSigner && !addressValid)} aria-describedby={intendedSigner && !addressValid ? signerErrorId : undefined} />{intendedSigner && !addressValid ? <small id={signerErrorId} className="field-error">Enter a valid {network === "celo" ? "Celo" : "Stacks"} address.</small> : null}</label> : null}
