  ...
  return (
    <div className="modal-backdrop" role="presentation">
      <section className="wizard" role="dialog" aria-modal="true" aria-labelledby="wizard-title">
        <header><div><span className="eyebrow">New CoSign · {step}/3</span><h2 id="wizard-title">{step === 1 ? "What happened?" : step === 2 ? "Add the signal." : "Review before publishing."}</h2></div><button type="button" className="icon-button" onClick={onClose} aria-label="Close" disabled={submitting}><X aria-hidden="true" /></button></header>
        ...
        {step === 2 ? <div className="form-stack">
          ...
          <fieldset><legend>Who can co-sign?</legend><div className="segmented"><button type="button" className={mode === "open" ? "active" : ""} aria-pressed={mode === "open"} onClick={() => setMode("open")}><Radio size={17} aria-hidden="true" /> Open link</button><button type="button" className={mode === "addressed" ? "active" : ""} aria-pressed={mode === "addressed"} onClick={() => setMode("addressed")}><LockKeyhole size={17} aria-hidden="true" /> One wallet</button></div></fieldset>
